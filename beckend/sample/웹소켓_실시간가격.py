...
class AccountMonitor:
    ...
    def _send_signed_request(self, method, path, params=None):
        """서명된 API 요청을 보내는 범용 함수"""
        if params is None:
            params = {}
        ...
        try:
            ...
            try:
                ...
                print(f"❌ JSON 디코딩 실패. 응답: {response.text}")
                return {"error": "JSONDecodeError", "raw_response": response.text}
                
        except requests.exceptions.RequestException as e:
            print(f"❌ API 요청 오류 ({method} {path}): {e}")
    ...
    def get_listen_key(self):
        ...
        result = self._send_signed_request("POST", path)
        ...
        if result and "listenKey" in result:
            ...
            print(f"❌ listenKey 발급 실패: {result.get('error', '알 수 없는 오류')}")
            return False
